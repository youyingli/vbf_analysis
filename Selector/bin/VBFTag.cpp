...
int main( int argc, char* argv[] ) {
    ...
    try {
        ...
        desc.add_options()
            ...
            ("outdir,o",po::value<string>(&outdir_)   ,"Output directory"                )
        ;
...
//------------------------------------------------------------------------------------------------------------------
    ...
    SystLabels.emplace_back("");//Still need normal 
    ...
    for (const auto& it : Total) {
        ...
        for (const auto& SystLabel_it : SystLabels) {
            ...
            if (dumpSysts && Name != "Data") selector.SetSystLabel(SystLabel_it);
            selector.selectLoop(isSideBand);
            if (!(dumpSysts && Name != "Data")) break;
        }
    }
